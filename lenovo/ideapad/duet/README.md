...
- **The GCE command queue only works with `clk_ignore_unused`.**
  `mediatek-drm` routes its register updates through the GCE mailbox
  (CMDQ) when that mailbox is registered by the time the CRTC is
  created, and falls back to CPU writes otherwise. Once
  `clk_disable_unused` has run (about a second into boot) the engine
  never executes a packet: `mtk_crtc 0 CMDQ execute command timeout!`
  followed by `flip_done timed out` every ten seconds and a lit panel
  that never shows a frame — and some minutes later the block stops
  answering on the bus, so a late `modprobe mtk_cmdq_mailbox` hangs the
  whole SoC. With unused clocks left on, the same kernel drives the
  display through the GCE with no timeouts. Which clock the engine
  depends on is not known: the `gce` node claims only `CLK_INFRA_GCE`,
  and the obvious candidate `CLK_INFRA_GCE_26M` is not it on its own
  (gating it, or all other unused infracfg gates, on a running GCE has
  no effect); the topckgen/apmixedsys/PLL candidates remain untested.
  The profile passes `clk_ignore_unused` and loads the mailbox from the
  initrd (plus a `softdep` for the udev path), so the display takes the
  CMDQ path on every boot. The CPU path works too, but its register
  writes have to finish inside the panel's ~240 µs vertical blank and
  the CPU idle states take longer than that to exit, so it tears and
  flickers on every cursor move.
- **Scanout buffers come from CMA.** The kernel's default 32 MiB pool
  holds three 1920x1200 framebuffers; a compositor and a browser exhaust
  it in seconds, after which `mtk_gem_create` fails and the display
  freezes while the rest of the system keeps running. The profile passes
  `cma=256M`.
